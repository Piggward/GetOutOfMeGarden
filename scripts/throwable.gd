...
func _input(event):
	if (event.is_action_pressed("left_click") or event.is_action_pressed(extra_pick_up_action)) and player.can_pick_up() and mouse_enter:
		_setheld(true)
		linear_velocity = Vector2.ZERO
		angular_velocity = 0
		linear_damp = 0
	elif (event.is_action_released("left_click") or event.is_action_released(extra_pick_up_action)) and is_held:
